...
results.to_csv('XGBoost_random_search_results_01.csv', index=False)

y_test = random_search.predict_proba(test_data)
results_df = pd.DataFrame(data={'id':np.array([i for i in range(200000,200000+len(ypred))]), 'loan_status':y_test[:,1]})
results_df.to_csv('XGBoost_random_search_prediction_01.csv', index=False)
